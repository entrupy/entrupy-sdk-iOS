...
@property (nonatomic, weak) id<EntrupyCatalogInsightsDelegate> catalogInsightsDelegate;
...
/**
 * Fetches catalog insights for a given Entrupy ID by calling the backend route.
 *
 * This method retrieves comprehensive catalog insights including condition details,
 * marketplace listings, and other relevant information for the specified authentication result.
 *
 * The response will be delivered through the EntrupyCatalogInsightsDelegate callbacks:
 * - didFetchCatalogInsightsCompleteSuccessfully: Called when the request succeeds
 * - didFetchCatalogInsightsFailWithError: Called when the request fails
 *
 * @param entrupyID The authentication ID for which to fetch catalog insights
 *
 * @note This method requires valid authorization. Ensure the user is logged in before calling.
 * @note The response data can be parsed using EntrupyCatalogInsights struct with JSONSerialization.
 *
 * Example usage:
 * @code
 * // Set the delegate
 * entrupyApp.catalogInsightsDelegate = self
 *
 * // Fetch catalog insights
 * entrupyApp.fetchCatalogInsightsForItem(withEntrupyID: "ABC123")
 *
 * // In the delegate method, parse the response:
 * func didFetchCatalogInsightsCompleteSuccessfully(_ catalog: [AnyHashable : Any], forEntrupyID entrupyID: String) {
 *     do {
 *         let data = try JSONSerialization.data(withJSONObject: catalog, options: [])
 *         let decoder = JSONDecoder()
 *         let catalogInsights = try decoder.decode(EntrupyCatalogInsights.self, from: data)
 *         // Use catalogInsights...
 *     } catch {
 *         print("Error parsing catalog insights: \(error.localizedDescription)")
 *     }
 * }
 * @endcode
 */
- (void)fetchCatalogInsightsForItemWithEntrupyID:(NSString *_Nonnull)entrupyID;
